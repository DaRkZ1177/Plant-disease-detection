...
def clean_image(image):
    ...
    image = np.array(Image.fromarray(
        image).resize((512, 512), Image.LANCZOS))  # Use Image.LANCZOS instead of Image.ANTIALIAS
    
    # Adding batch dimensions to the image
    # You are setting :3 because sometimes users upload 4-channel images,
    image = image[np.newaxis, :, :, :3]  # Take the first 3 channels
